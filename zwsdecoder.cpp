...
{
	zwsdecoder_t *self = (zwsdecoder_t *)zmalloc(sizeof(zwsdecoder_t));
...
void zwsdecoder_process_buffer(zwsdecoder_t *self, zframe_t* data)
{
	...
	int buffer_length = (int)zframe_size(data);
	...
	while (i < buffer_length)
	{
		...
		case begin_payload:		
			...
			self->payload = (byte*)zmalloc(sizeof(byte) * self->payload_length);
...
static void zwsdecoder_process_byte(zwsdecoder_t *self, byte b)
{
	...
	case new_message:
		...
		self->opcode = (opcode_t)(b & 0xF); // opcode bit
